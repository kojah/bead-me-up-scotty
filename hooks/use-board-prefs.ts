...
function snapshot() {
  try {
    return globalThis.localStorage?.getItem(PREFS_KEY) || "";
  } catch {
    return "";
  }
}
function parse(raw: string): BoardPrefs {
  try {
    ...
    return {
      blockedColumn: stored?.blockedColumn === "always" ? "always" : "auto",
      sortMode: ["priority", "updated", "manual"].includes(stored?.sortMode)
        ? stored.sortMode
        : "manual",
