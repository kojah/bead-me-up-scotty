...
test("board sorting", async ({ browser, baseURL }) => {
  const base = baseURL;
  expect(base, "Playwright baseURL must be configured").toBeTruthy();

  const bead = (id: string, priority: number, updated_at: string, extra: Partial<Bead> = {}) =>
    beadSchema.parse({
      id,
      title: `Sorting ${id}`,
      status: "open",
  ...
  const orders: Record<string, string[]> = { ready: ["manual-a", "manual-z"] };
  ...
  try {
    ...
    const expectReady = async (ids: string[], message: string) =>
      expect(await readyIds(), message).toStrictEqual(ids);
    const drag = async (sourceId: string, targetId: string) => {
      ...
      const from = required(await source.boundingBox());
      const to = required(await target.boundingBox());
      expect(from && to, `drag targets must be visible: ${sourceId} → ${targetId}`).toBeTruthy();
    ...
    expect(await sort().inputValue(), "unknown saved mode falls back to manual").toBe("manual");
    await expectReady(
    ...
    expect(await sort().inputValue(), "selected sort mode survives reload").toBe("priority");
    await expectReady(
      ["p0-old", "p1-new", "a-tie", "z-tie", "p2-new", "moving", "manual-a", "manual-z"],
    ...
    expect(writes.length, "read-only board blocks drag writes").toBe(readOnlyWrites);
    ...
    await page.waitForTimeout(250);
    expect(writes.length, "priority within-column drag does not write an order or status").toBe(
    ...
    required(beads.find((candidate) => candidate.id === "moving")).status = "open";
    await page.reload();
    await sort().waitFor();
    await sort().selectOption("updated");
    ...
    await drag("p0-old", "p1-new");
    await page.waitForTimeout(250);
    expect(writes.length, "updated within-column drag does not write an order or status").toBe(
    ...
    expect(required(writes.at(-1)).body).toStrictEqual({
      columnId: "ready",
