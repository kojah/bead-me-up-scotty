...
test("notifications", async ({ browser, baseURL }) => {
  ...
  const projectData: Record<string, Bead[]> = {
    [projectOne]: [
  ...
  let activity: ReturnType<typeof activityItem>[] = [];
  const activityItem = (id: string, action: string, at: string) => ({
    id,
    issueId: id,
    title: required(projectData[projectOne].find((b) => b.id === id)).title,
    actor: "agent",
    origin: "agent",
    action,
    at,
  ...
  try {
    ...
    await page.route("**/api/p/**", async (route) => {
      ...
      const match = required(url.pathname.match(/\/api\/p\/([^/]+)\/(.*)$/));
    ...
    await page.evaluate((index) => {
      const n = window.__notifications[index];
      if (!n.onclick) throw Error("Notification click handler missing");
