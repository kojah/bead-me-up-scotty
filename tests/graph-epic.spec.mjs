...
test("graph epic", async ({ browser, baseURL }) => {
  ...
  const deep = Array.from({ length: 2500 }, (_, i) =>
    bead(`chain-${i}`, { dependencies: i ? [dep(`chain-${i - 1}`, "blocks")] : [] }),
  );
  assert.equal(
    graphDependencyLayers(deep).get("chain-2499"),
    2499,
    "deep dependency chains avoid call-stack limits",
  );
  const cycle = [
    bead("a", { dependencies: [dep("b", "blocks")] }),
    bead("b", { dependencies: [dep("a", "blocks")] }),
    bead("c", { dependencies: [dep("a", "blocks")] }),
  ];
  ...
  assert.deepEqual(
    [...graphDependencyLayers([...cycle].reverse())],
    [...layers],
    "layers do not depend on input order",
  );
  assert.ok(
    buildEpicGraphScope(
      beads.filter((b) => b.id !== "archived"),
      "epic",
    ).insideIds.has("sub-6"),
  );
  const writes = [];
  ...
  try {
    ...
    for (let i = 0; i < rectangles.length; i++)
      for (let j = i + 1; j < rectangles.length; j++) {
        const a = rectangles[i],
          b = rectangles[j];
        if (
          beads.find((n) => n.id === a.id)?.issue_type === "epic" ||
          beads.find((n) => n.id === b.id)?.issue_type === "epic"
        )
          continue;
        assert.ok(
          a.right <= b.left + 1 ||
    ...
    assert.deepEqual(
      writes.at(-1),
      { id: "sub-4", depends_on_id: "outside", type: "blocks" },
      "epic mode draws prerequisite to dependent",
    );
    await scope().selectOption("");
    await node("solo").waitFor();
    ...
    assert.deepEqual(
      writes.at(-1),
      { id: "solo", depends_on_id: "sub-5", type: "blocks" },
      "whole graph draws prerequisite to dependent",
    );
    await scope().selectOption("epic");
    ...
    assert.equal(
      ...
      "",
      "archiving the selected epic returns to All beads",
    );
