...
test("dependency rows", async ({ browser, baseURL }) => {
  ...
  for (const text of [
    ...
    expect(
      await dependencies.getByText(text, { exact: true }).count(),
      `shows ${text}`,
    ).toBeTruthy();
  }
  ...
  expect(await missing.isDisabled(), "missing targets cannot be opened").toBe(true);
  expect(
  ...
  expect(deletes.shift(), "incoming removal must remove the dependent's edge").toStrictEqual({
    path: "/api/p/demo/beads/in-block/deps",
    body: { depends_on_id: "center" },
  });
  ...
  expect(deletes.shift(), "outgoing removal keeps the current bead as source").toStrictEqual({
    path: "/api/p/demo/beads/center/deps",
    body: { depends_on_id: "out-related" },
  });
