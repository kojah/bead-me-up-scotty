...
const bead = (id, extra = {}) => ({ id, title: `Task ${id}`, status: 'open', issue_type: 'task',
  priority: 2, labels: [], dependencies: [], description: 'A mobile test task.', ...extra });
const fixtures = [bead('epic', { issue_type: 'epic', title: 'Ship a coherent mobile experience' }),
  bead('nested', { issue_type: 'epic', dependencies: [dep('epic')] }),
  bead('first', { title: 'A deliberately very long title '.repeat(12), dependencies: [dep('nested')] }),
  ...
  await page.route('**/api/**', async route => {
    ...
    if (req.method() !== 'GET') {
      ...
      if (path.endsWith('/beads') && req.method() === 'POST') {
        const created = bead('created', req.postDataJSON()); beads.push(created);
        return route.fulfill({ json: created });
      }
      return route.fulfill({ json: {} });
    }
    if (path.endsWith('/beads')) return route.fulfill({ json: { beads, meta: {
      kind: 'demo', humanActor: 'tester', humanAllowlist: ['tester'], pollIntervalMs: 300000,
    } } });
    if (/\/beads\/[^/]+$/.test(path)) return route.fulfill({ json: beads.find(b => path.endsWith('/' + b.id)) });
...
async function noOverflow(page) {
  ...
  if (await header.count()) assert.ok(await header.evaluate(e => e.scrollWidth <= e.clientWidth + 1), 'toolbar overflow');
}
async function navigate(page, name) {
  await page.getByRole('button', { name: 'Open navigation', exact: true }).click();
  await page.getByRole('navigation', { name: 'Views' }).getByRole('button', { name: new RegExp(`^${name}(?: \\d+)?$`) }).click();
...
try {
  for (const width of [360, 390, 430]) {
    ...
    assert.equal(await page.locator('.task-row').first().evaluate(e => getComputedStyle(e).touchAction), 'pan-y');
    ...
    assert.equal(rect.x, 0); assert.equal(rect.width, width);
    ...
    assert.ok((await page.locator('.react-flow').boundingBox()).height >= 350, 'graph has usable canvas height');
    ...
    assert.notEqual(await page.locator('.react-flow__viewport').getAttribute('style'), transform, 'canvas pans by touch');
    if (width === 390) await page.screenshot({ path: '/tmp/scotty-mobile-graph.png' });
    for (const view of ['Board', 'Epics', 'Focus', 'Activity', 'Needs You', 'Insights', 'Settings']) {
    ...
    await context.close();
    console.log(`PASS mobile ${width}px: default view, filters/reload, details, graph, navigation, project switch, resize`);
  }
  ...
  assert.equal(create.width, 390); assert.equal(create.x, 0);
