...
export function containerLayout(
  ...
  const owners = new Map(
    [...fullOwners].filter(([id, owner]) => visibleIds.has(id) && visibleIds.has(owner)),
  );
  ...
  function place(owner: string, startY: number): number {
    ...
    for (const epic of members.filter((b) => b.issue_type === "epic")) {
      ...
      const x = childBoxes.length
        ? Math.min(...childBoxes.map((b) => b.x)) - 28
        : (layers.get(epic.id) ?? 0) * 290;
  ...
  return order.map((bead) => {
    ...
    const descendants = allBeads.filter((b) => {
      ...
      while (owner) {
        if (owner === bead.id) return true;
        owner = fullOwners.get(owner);
      }
      return false;
    });
    ...
    return {
      ...
      data: {
        ...
        completed: descendants.filter((b) => b.status === "closed").length,
      },
    };
  });
}
