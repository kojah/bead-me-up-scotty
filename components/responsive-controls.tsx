...
export function ResponsiveControls({ children, title = "Filters", count = 0 }: {
  children: React.ReactNode; title?: string; count?: number;
}) {
  ...
  return <Sheet open={open} onOpenChange={setOpen}>
    <SheetTrigger className="control-button" aria-label={title}>
      {title}{count > 0 && <span className="rounded-full bg-[var(--brand-weak)] px-2 text-[var(--brand)]">{count}</span>}
    </SheetTrigger>
    <SheetContent side="bottom" className="mobile-controls max-h-[85dvh] overflow-y-auto rounded-t-2xl p-5 pb-[max(20px,env(safe-area-inset-bottom))]">
