...
function DrawerBody({
  ...
  function onDescriptionKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    const transforms: Record<string, typeof bold> = { b: bold, i: italic, k: link };
    const transform = transforms[e.key];
    if (!transform) return;
    e.preventDefault();
    applyTransform(descRef.current, descDraft, setDescDraft, transform);
  }
  ...
  const { notes, design, acceptance, closeReason, comments, activity } = detailMetadata(bead);
...
function detailMetadata(bead: Bead) {
  ...
  return { notes, design, acceptance, closeReason, comments, activity };
}
