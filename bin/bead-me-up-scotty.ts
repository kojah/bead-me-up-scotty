...
type Flags = { open: boolean; help: boolean; port?: number; host?: string; portExplicit: boolean };
function parseFlags(argv: string[]) {
  const opts: Flags = { open: true, help: false, portExplicit: false };
...
function parseArgs(argv: string[]) {
  ...
  return { ...opts, port: opts.port, host: opts.host };
}
...
async function pickPort(start: number, explicit: boolean) {
...
function waitForReady(host: string, port: number, timeoutMs = 30000) {
...
async function targetPath(host: string, port: number) {
  try {
    if (!fs.existsSync(path.join(invocationCwd, ".beads"))) return "/";
    const body = JSON.stringify({ path: invocationCwd });
    const entry = await new Promise<{ id?: string }>((resolve, reject) => {
